...
type InjectedPromptDraft = VeoFivePartDraft & {
  concept?: string;
};
...
export type VeoPromptOptions = {
  ...
  lockLens?: boolean;
  lockStyle?: boolean;
  injectPromptDraft?: InjectedPromptDraft;
};

const safeJoin = (val: unknown): string => {
  if (Array.isArray(val)) return val.filter((v): v is string => typeof v === 'string' && v.length > 0).join(', ');
