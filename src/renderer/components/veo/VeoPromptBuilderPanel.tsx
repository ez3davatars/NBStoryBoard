...
const VeoPromptBuilderPanel = forwardRef<VeoPromptBuilderRef, VeoPromptBuilderPanelProps>((_, ref) => { // Removed analysisSpec from props
 ...
 // Auto-save draft changes to the Active Shot (Debounced)
 useEffect(() => {
 ...
 activeShot?.id, // Ensure we are saving to correct shot
 ...
 if (negativePrompt) {
 combined += ` --no ${negativePrompt.trim()}`;
 }
 ...
 dispatch({ type: 'ADD_LOG', payload: { message: "Combined Prompt Copied", type: 'info' } });
 }
 };
