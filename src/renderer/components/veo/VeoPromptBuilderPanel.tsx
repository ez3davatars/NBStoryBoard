...
import { Wand2, Loader2, ChevronDown, Check, Maximize2 } from 'lucide-react';
...
const VeoPromptBuilderPanel = forwardRef<VeoPromptBuilderRef, VeoPromptBuilderPanelProps>((_, ref) => { // Removed analysisSpec from props
    ...
    return (
        <div className="flex flex-col h-full text-gray-200">
            <div className="flex-1 h-full min-h-0 overflow-y-auto no-scrollbar p-4 pb-6 space-y-4">
                ...
                >
                    <AutoGrowTextarea
                        value={concept}
                        onChange={(e: { target: { value: React.SetStateAction<string>; }; }) => setConcept(e.target.value)}
            ...
            <div className="shrink-0 p-4 border-t border-white/10 bg-black/40 backdrop-blur-md flex flex-col gap-2">
                <button
                    ...
                    className="w-full py-2.5 rounded-full flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all bg-white/5 hover:bg-white/10 border border-white/10 text-white disabled:opacity-50"
                >
                ...
                <button className="w-full py-2.5 rounded-full flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all bg-gradient-to-r from-yellow-400 to-amber-500 text-black hover:scale-[1.02] shadow-[0_0_15px_rgba(250,204,21,0.3)]">
                    Generate
                </button>
            </div>
            ...
            {activeEditor && (
                <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-8 backdrop-blur-sm">
                    <div className="bg-[#0b1220] border border-white/20 rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6">
                            <AutoGrowTextarea
                                ...
                                minRows={10}
                                maxRows={30}
                                className="text-sm shadow-inner"
                                autoFocus
                            />
                        </div>
                        <div className="px-6 py-4 bg-black/40 border-t border-white/10 flex justify-end">
                            <button
                                onClick={() => setActiveEditor(null)}
                                className="bg-yellow-500 hover:bg-yellow-400 text-black px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(250,204,21,0.2)]"
                            >
                                Done
                            </button>
                        </div>
