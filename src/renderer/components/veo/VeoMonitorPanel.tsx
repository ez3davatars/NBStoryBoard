...
export default function VeoMonitorPanel() {
 ...
 if (!activeShot) {
 dispatch({ type: 'ADD_LOG', payload: { message: "Select or Create a Shot from the panel first.", type: 'error' } });
 return;
 }
 ...
 <div className="mx-auto w-full max-w-[1100px] max-h-full flex flex-col aspect-video bg-black/40 rounded-2xl border border-white/10 overflow-hidden relative">
 ...
 ${activePlate === 'start' ? 'bg-white/15 text-white ' : 'text-gray-500 hover:text-gray-300'}`}
 >
 Start Plate
 </button>
 <button
 ...
 className={`px-4 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all 
 ${activePlate === 'end' ? 'bg-white/15 text-white ' : 'text-gray-500 hover:text-gray-300'}`}
 >
 End Plate
 </button>
 </div>
 ...
 ${activePlate === 'start' ? 'border-green-500 -[0_0_15px_rgba(34,197,94,0.3)] ring-2 ring-green-500/20' : 'border-white/10 opacity-60 hover:opacity-100 hover:border-white/30'}`}
 >
 ...
 onClick={() => setActivePlate('end')}
 className={`relative h-16 w-28 rounded-md overflow-hidden border-2 transition-all flex-shrink-0 bg-[#0d131f]
 ${activePlate === 'end' ? 'border-blue-500 -[0_0_15px_rgba(59,130,246,0.3)] ring-2 ring-blue-500/20' : 'border-white/10 opacity-60 hover:opacity-100 hover:border-white/30'}`}
 >
