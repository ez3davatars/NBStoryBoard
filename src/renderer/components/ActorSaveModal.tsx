        ...
        >
          ...
          >
            <h3 className="text-lg font-black text-white uppercase tracking-widest mb-2 flex items-center gap-2">
              <FolderInput className="w-5 h-5 text-emerald-500" /> {title}
            </h3>
              ...
              {STUDIO_FOLDERS.map((folder) => {
                ...
                return (
                  <button
                    key={folder.id}
                    onClick={() => {
                      const finalName = hideNameInput ? initialName : name.trim();
                      if (finalName) onSave(finalName, folder.id);
                    }}
                    disabled={!(hideNameInput ? initialName : name.trim())}
                    className={`group relative h-24 w-full rounded-xl overflow-hidden border border-white/10 transition-all text-left ${
                      (hideNameInput ? initialName : name.trim()) ? "hover:scale-[1.02] hover:border-emerald-500 cursor-pointer" : "opacity-50 cursor-not-allowed"
                    }`}
                  >
                    {/* Background Image */}
                    {activeImage ? (
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/40 to-transparent flex flex-col justify-center px-6">
                      <div>
                        <h3 className={`text-xl font-black italic tracking-tighter uppercase transition-colors leading-none ${(hideNameInput ? initialName : name.trim()) ? "text-white group-hover:text-emerald-400" : "text-gray-400"}`}>
                          {folder.label}
                        </h3>
                        <div className="flex items-center gap-2 mt-1">
                          <p className={`text-[10px] font-bold border-l-2 pl-2 border-emerald-500 ${(hideNameInput ? initialName : name.trim()) ? "text-gray-300" : "text-gray-500"}`}>
                            {folder.description}
                          </p>
                        </div>
                      </div>
