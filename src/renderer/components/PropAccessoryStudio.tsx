...
const PropAccessoryStudio = () => {
  ...
  const handleAddToCast = () => {
    ...
    dispatch({ type: 'ADD_CAST', payload: {
      id: `propfit-${Date.now()}`,
      url: finalUrl,
      tag: 'front',
  ...
  const handleSaveToActors = async () => {
    ...
    try {
        ...
        await writable.write(blob);
        await writable.close();
        dispatch({ type: 'ADD_ACTOR_LIBRARY', payload: {
      ...
      <div className="w-80 border-r border-gray-800 bg-[#18181b] flex flex-col shadow-xl">
        ...
        <div className="flex-grow overflow-y-auto p-4 grid grid-cols-2 gap-2">
            {state.propItems.map(item => (
              <button key={item.id} onClick={() => setSelectedProp(item)} className={`aspect-square rounded-lg border overflow-hidden transition-all group relative ${selectedProp?.id === item.id ? 'border-blue-500 border-2' : 'border-gray-800 hover:border-gray-600'}`}>
                <img src={item.url} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button onClick={(e) => { e.stopPropagation(); dispatch({ type: 'SET_INSPECT_IMAGE', payload: item.url }); }} className="bg-blue-500/80 p-1.5 rounded-full"><Maximize className="w-3.5 h-3.5" /></button>
                </div>
              </button>
            ))}
        </div>
                        ...
                        {appliedImage && (
                            <div className="absolute top-6 right-6 flex flex-col gap-3 z-20 bg-black/60 p-4 rounded-3xl border border-white/10 backdrop-blur-md">
                                <div className="flex items-center justify-between gap-6">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={removeApplyBg} onChange={(e) => setRemoveApplyBg(e.target.checked)} className="w-4 h-4 accent-blue-500 rounded" /><Eraser className="w-3.5 h-3.5" /> Remove BG</label>
                                    {applyMask && <label className="text-[9px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={applyAiMaskActive} onChange={(e) => setApplyAiMaskActive(e.target.checked)} className="w-3.5 h-3.5 accent-blue-500 rounded" /><Sparkles className="w-3 h-3" /> AI Masking</label>}
                                </div>
                                {removeApplyBg && (
