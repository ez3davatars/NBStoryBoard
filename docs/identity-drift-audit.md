...
| **Model substitution (invariant #5)** | RULED OUT as *recent* cause | `normalizeImageGenerationModel` always returns the pinned image model (`NANO_BANANA_2_IMAGE_MODEL`, now the GA `gemini-3.1-flash-image`); `constants/generationModels.ts` unchanged by the checkpoint. Pinned for many commits; never falls back to a text model. |
